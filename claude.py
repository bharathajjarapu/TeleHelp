...
async def location(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_location:
        ...
        if any(word in llm_reply.lower() for word in doctors):
            ...
            await context.bot.send_message(chat_id='5882873021', text=admin_message)
        ...
        if any(word in llm_reply.lower() for word in fireworks):
            admin_message = f"Send a Fire Brigade for : \n{llm_reply} \nLocation : {user_location}\n"
            await context.bot.send_message(chat_id='5882873021', text=admin_message)
            await update.message.reply_text("Thank you for sharing your location. Fire Brigade will arrive shortly")
